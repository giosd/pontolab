...
import type { HoursByUserItem } from "@/types";

interface HoursByUserChartProps {
  data: HoursByUserItem[];
}

export function HoursByUserChart({ data }: HoursByUserChartProps) {
  return (
    <Card>
      <h2 className="mb-4 text-base font-medium text-[#1E5F7A]">Horas por usuário</h2>
      {data.length === 0 ? (
        <EmptyChartMessage />
      ) : (
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              />
              <Bar dataKey="hours" fill="#7CC7E8" radius={[8, 8, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
